...
Scripts to filter the data to get df.incorp.liq.csv from lit_dat.xlsx: ../scripts. The script 'main.R' runs the rest. 
...
Database from Pedersen et al. (2022): ../data/separation_dat_Pedersen2022.xlsx
...
Scripts to filter the data to get df.solid.csv from lit_dat.xlsx: ../scripts. The script 'main.R' runs the rest. 
The reductions in these studies are in line with the GD from 2014, therefore, we did not find that there was evidence to change these. 
The numbers from the 2014 GD are used in this revised version without any changes. 
